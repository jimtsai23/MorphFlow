...
class DirectVoxGO(torch.nn.Module):
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, render_alpha=None, render_rgb=None, **render_kwargs):
        '''Volume rendering'''
        ...
        if self.pre_act_density:
            ...
            # alpha = self.grid_sampler(rays_pts, render_alpha)
            alpha[~mask_outbbox] = self.grid_sampler(rays_pts[~mask_outbbox], render_alpha)
        ...
        if render_rgb is not None:
            # k0 = self.grid_sampler(rays_pts, render_rgb)
            k0[mask] = self.grid_sampler(rays_pts[mask], render_rgb)
            rgb = k0
        else:
